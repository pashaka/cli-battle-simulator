...
### Юниты:
- **Class Unit** - базовый класс юнита, хранит id, hp, координаты, имя, доступные действия, требует реализации `getActionTypesOrder()` - возвращает порядок действий юнита, чисто виртуальный метод. `std::optional<uint32_t> hp` - управляет можно ли юнит атаковать. Если значение есть и оно 0 - юнит мертв. У мины значения нет, но после взрыва становится 0
